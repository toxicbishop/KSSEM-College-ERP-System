...
export function AnnouncementsCard({ announcements }: AnnouncementsCardProps) {
  ...
  return (
    <Card className="shadow-md hover:shadow-lg transition-shadow h-full bg-[#222B36] border-[#2a3441] text-white">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-white">
          <Megaphone className="h-5 w-5 text-[#2dd4bf]" />
          {loadingSettings ? "Loading..." : systemSettings?.announcementTitle}
        </CardTitle>
      </CardHeader>
      <CardContent className="h-[calc(100%-70px)]">
        {loadingSettings ? (
          <p className="text-[#8A99BB]">Loading announcement...</p>
        ) : (
