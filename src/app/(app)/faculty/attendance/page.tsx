...
export default function FacultyAttendancePage() {
  ...
  const [uniqueBatchesInClassroom, setUniqueBatchesInClassroom] = useState<
    string[]
  >([]);
  const [selectedBatchFilter, setSelectedBatchFilter] = useState<string>(
    WHOLE_CLASS_FILTER_VALUE,
  );
  ...
  const [loadingPreviousAttendance, setLoadingPreviousAttendance] =
    useState(false);
  ...
  const [analysisResult, setAnalysisResult] =
    useState<AttendanceAnalysisOutput | null>(null);
  ...
  const lowAttendanceStudents = useMemo(() => {
    ...
    return Object.keys(studentStats)
      .map((studentId) => {
  ...
  const pieChartData = useMemo(() => {
    if (reportRecords.length === 0) return [];
    const presentCount = reportRecords.filter(
      (r) => r.status === "present",
    ).length;
    const absentCount = reportRecords.filter(
      (r) => r.status === "absent",
    ).length;
    ...
    return [
      ...
      },
      {
        name: "Absent",
        value: absentCount,
        percentage: ((absentCount / total) * 100).toFixed(1),
      },
    ];
  }, [reportRecords]);
    ...
    try {
      ...
      if (previousRecords && previousRecords.length > 0) {
        ...
        currentStudents.forEach((student) => {
          ...
          previousStatus[student.userId] = record
      ...
      toast({
        title: "Error",
        description:
          (error as Error).message || "Could not load previous attendance.",
        variant: "destructive",
      });
      setIsEditing(false);
    } finally {
  ...
  const filteredStudentsToDisplay = useMemo(() => {
    ...
    if (selectedBatchFilter !== WHOLE_CLASS_FILTER_VALUE) {
      studentsToDisplay = currentStudents.filter(
        (student) => student.batch === selectedBatchFilter,
      );
    }
  ...
  useEffect(() => {
    ...
    if (filteredStudentsToDisplay.length > 0) {
      const allPresent = filteredStudentsToDisplay.every(
    ...
    attendanceStatus,
    filteredStudentsToDisplay,
    loadingStudents,
    loadingPreviousAttendance,
  ]);
  ...
  const handleSubmitAttendance = async () => {
    if (!user || !clientAuth?.currentUser) {
      toast({
        title: "Authentication Error",
        description: "Cannot submit attendance.",
        variant: "destructive",
      });
      return;
    }
    if (!selectedClassroomId || !selectedDate || !lectureSubjectTopic.trim()) {
      toast({
        ...
        description: "Please select classroom, date, and enter Lecture Topic.",
        variant: "destructive",
      });
      return;
    }
    if (!selectedClassroomDetails) {
      toast({
        title: "Error",
        description: "Selected classroom details not found.",
        variant: "destructive",
      });
      return;
    }
    if (!allStudentsHaveDefinedStatus) {
      toast({
        ...
        description: "Ensure every student is marked present or absent.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDeleteAttendance = async () => {
    if (
      ...
      toast({
        title: "Error",
        description: "Classroom or date not selected.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (fetchedRecords.length === 0) {
        toast({
          ...
          description:
            "No attendance records were found for the selected classroom and date range.",
        });
      } else {
        // Trigger AI analysis
        try {
          ...
          toast({
            ...
            description:
              "Could not generate AI insights for the attendance data.",
            variant: "default",
          });
      ...
      toast({
        ...
        description:
          (error as Error).message || "Could not retrieve attendance records.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleDownloadReport = useCallback(() => {
    if (!studentStats || currentStudents.length === 0) {
      toast({
        ...
        description: "Please fetch records before downloading a report.",
        variant: "destructive",
      });
      return;
    }
    ...
    const dataForCsv = currentStudents
      ...
      .map((student, index) => {
        ...
        const percentage =
          studentStat && studentStat.total > 0
        ...
        uniqueLectures.forEach((lecture) => {
          ...
          rowData[lectureKey] =
            attendanceMap.get(
              `${student.userId}-${lecture.date}-${lecture.lectureName}`,
            ) || "A";
        });
  ...
  const handleDownloadDefaulterReport = useCallback(() => {
    if (lowAttendanceStudents.length === 0) {
      toast({
        title: "No Data to Export",
        description: "There are no students in the low attendance list.",
        variant: "destructive",
      });
      return;
    }
          ...
          <TabsContent value="mark" className="mt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select a classroom and date to mark new attendance or edit a
                  previous record.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    ...
                    {loadingClassrooms ? (
                      ...
                      <Select
                        ...
                        <SelectTrigger id="classroom-mark">
                          <SelectValue
                            placeholder={
                              classrooms.length > 0
                                ? "Choose a classroom"
                  ...
                  <div>
                    ...
                    <Input
                      id="lectureSubjectTopic"
                      placeholder="e.g., CH-5 Thermodynamics"
                      value={lectureSubjectTopic}
                      onChange={(e) => setLectureSubjectTopic(e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      ...
                      disabled={
                        !selectedClassroomId ||
                        uniqueBatchesInClassroom.length === 0
                      }>
                ...
                <CardHeader className="bg-muted/50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      {selectedClassroomDetails?.subject || "N/A"}
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  {loadingStudents || loadingPreviousAttendance ? (
                    ...
                    <>
                      <div className="bg-muted/30 p-3 flex justify-between items-center text-sm font-medium">
                        <div>
                          ...
                          <span className="text-primary">
                            {filteredStudentsToDisplay.length}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span>Present:</span>
                          ...
                          <Table className="min-w-full">
                            <TableHeader>
                              <TableRow className="bg-muted/10">
                                <TableHead className="w-[80px] px-3 py-2 text-center border">
                                  Sr. No.
                                </TableHead>
                                  ...
                                  <TableRow key={student.userId}>
                                    ...
                                    <TableCell className="px-3 py-2 text-center border">
                                      <div className="flex items-center justify-center">
                                        <Checkbox
                                          id={`attendance-table-${student.userId}`}
                                          checked={
                      ...
                      <div className="p-4 flex flex-col sm:flex-row items-center justify-end gap-2 border-t">
                        ...
                        <Button
                          ...
                          disabled={
                            isSubmitting ||
                            !allStudentsHaveDefinedStatus ||
                            !lectureSubjectTopic.trim()
                          }
                          className="w-full sm:w-auto">
                          {isSubmitting && (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ...
                    <p className="text-muted-foreground text-center p-6">
                      ...
                      !loadingStudents
                        ...
                        : !loadingClassrooms && !loadingStudents
                          ? "No students match the current batch filter."
                          : "Select a classroom to load students."}
                    </p>
                  )}
                </CardContent>
              </Card>
          ...
          <TabsContent value="reports" className="mt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select a classroom and a report type to view records and
                  generate reports.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                  ...
                  <div className="md:col-span-1">
                    ...
                    {loadingClassrooms ? (
                      ...
                      <Select
                        ...
                        onValueChange={setSelectedClassroomId}
                        disabled={classrooms.length === 0}>
                        <SelectTrigger id="classroom-report">
                          <SelectValue
                            placeholder={
                              classrooms.length > 0
                                ? "Choose a classroom"
                ...
                <div className="flex justify-end">
                  <Button
                    ...
                    disabled={isFetchingRecords || !selectedClassroomId}
                    className="w-full sm:w-auto">
                    {isFetchingRecords ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : (
                        ...
                        <CardHeader>
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <Button
                              variant="secondary"
                              size="sm"
                              onClick={handleDownloadDefaulterReport}
                          ...
                          <CardDescription>
                            This report shows students with attendance below the
                            set threshold for the selected date range.
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                          ...
                          <div className="overflow-x-auto">
                            {lowAttendanceStudents.length > 0 ? (
                              ...
                              <p className="text-center text-muted-foreground py-4">
                                No students are below the {attendanceThreshold}%
                                threshold.
                              </p>
                            )}
                          </div>
                                ...
                                <AccordionTrigger className="hover:no-underline">
                                  <div className="flex w-full items-center justify-between text-left">
                                    <span className="font-semibold text-primary">
                                      {format(new Date(dateGroup.date), "PPP")}
                                    </span>
                                    <span className="text-sm text-muted-foreground ml-4 shrink-0">
                                      {dateGroup.lectures.length} lecture(s)
                                    </span>
                                  </div>
                                </AccordionTrigger>
                                <AccordionContent>
                                          ...
                                          {isMobile ? (
                                            <div className="space-y-2">
                                              {lecture.records
                                                ...
                                                .map(
                                                  (
                                                    record: LectureAttendanceRecord,
                                                  ) => (
                                                    <div
                                                      key={record.id}
                ...
                {!isFetchingRecords &&
                  ...
                  reportRecords.length === 0 && (
                    <div className="text-center text-muted-foreground pt-6">
                      <p>
                        No records to display. Select a classroom and date
                        range, then click "View Records".
                      </p>
                    </div>
                  )}
              </CardContent>
