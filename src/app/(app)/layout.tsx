...
export default function AppLayout({
  ...
  const [isStudentSidebarCollapsed, setIsStudentSidebarCollapsed] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      if (
        ...
        console.log(
          `AppLayout (No User Branch): Attempting to redirect from ${pathname} to /signin.`,
        );
        router.push("/signin");
      }
      return;
    ...
    if (user.email === "admin@gmail.com") {
      ...
      console.error(
        "Firestore db instance is not available for role checking. Defaulting to student role.",
      );
      setUserRole("student");
      setCheckingRole(false);
    }
                ...
                {[...Array(6)].map((_, i) => (
                  <Skeleton
                    ...
                    className={cn(
                      ...
                      isStudentSidebarCollapsed && "w-10 mx-auto",
                    )}
                  />
                ))}
              </nav>
