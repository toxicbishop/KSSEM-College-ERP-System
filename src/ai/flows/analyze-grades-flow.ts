...
const analyzeGradesFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (input.length === 0) {
      return {
        overallSummary:
          "No grades are available yet to analyze. Keep up the good work and your grades will appear here as they are entered!",
        strengths: [],
        areasForImprovement: [],
      };
    }
    ...
    try {
      ...
      if (!output) {
        throw new Error(
          "The AI model did not return a valid analysis. Please try again.",
        );
      }
      ...
      return {
        overallSummary:
          "AI analysis is currently unavailable. Please check your API key or try again later.",
        strengths: [],
        areasForImprovement: [],
      };
    }
