...
export const GradeAnalysisOutputSchema = z.object({
  overallSummary: z
    .string()
    .describe(
      "A brief, encouraging overall summary of the student's performance based on the grades.",
    ),
  strengths: z
    .array(z.string())
    .describe(
      "A list of subjects or areas where the student is performing well.",
    ),
  areasForImprovement: z
    .array(z.string())
    .describe(
      "A list of subjects or areas where the student could focus on improving.",
    ),
});
...
export async function getGrades(studentId: string): Promise<Grade[]> {
  if (adminInitializationError) {
    console.error(
      "getGrades SA Error: Admin SDK init failed:",
...
export async function getGradesForStudent(
  ...
  if (adminInitializationError) {
    console.error(
      "getGradesForStudent SA Error: Admin SDK init failed:",
      adminInitializationError.message,
    );
    throw new Error("Server error: Admin SDK initialization failed.");
  }
  ...
  try {
    ...
    console.error(
      "getGradesForStudent SA Error: Invalid faculty ID token",
      error,
    );
...
export async function updateStudentGrade(
  ...
  if (adminInitializationError) {
    console.error(
      "updateStudentGrade SA Error: Admin SDK init failed:",
      adminInitializationError.message,
    );
    throw new Error("Server error: Admin SDK initialization failed.");
  }
  ...
  try {
    ...
    console.error(
      `Error updating grade for student ${studentId} in course ${courseName}:`,
      error,
    );
