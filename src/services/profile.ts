...
export async function getStudentProfile(
  ...
  if (adminInitializationError) {
    console.error(
      "[ServerAction:getStudentProfile] Admin SDK init failed:",
  ...
  if (!adminDb || !adminAuth) {
    console.error(
      "[ServerAction:getStudentProfile] Admin DB or Auth not initialized.",
    );
  ...
  try {
    ...
    if (userDocSnap.exists) {
      ...
      return {
        ...
        // Academic Details
        enrollmentNumber:
          userData.enrollmentNumber || userData.studentId || targetUid,
        ...
        admissionLetterUrl:
          userData.admissionLetterUrl || "#view-admission-letter",
        ...
        migrationCertificateUrl:
          userData.migrationCertificateUrl || "#view-migration-cert",
        bonafideCertificateUrl:
          userData.bonafideCertificateUrl || "#download-bonafide",
        uploadedPhotoUrl: userData.uploadedPhotoUrl || "",
        uploadedSignatureUrl: userData.uploadedSignatureUrl || "",
        ...
        internalExamTimetableUrl:
          userData.internalExamTimetableUrl || "#view-internal-timetable",
        externalExamTimetableUrl:
          userData.externalExamTimetableUrl || "#view-external-timetable",
        resultsAndGradeCardsUrl:
          userData.resultsAndGradeCardsUrl || "#view-results",
        revaluationRequestStatus: userData.revaluationRequestStatus || "None",
        revaluationRequestLink:
          userData.revaluationRequestLink || "#request-revaluation",
    ...
    console.error(
      "Error fetching student profile from Firestore (Admin SDK):",
      error,
    );
    throw error;
  }
...
export async function updateStudentProfile(
  ...
  if (adminInitializationError) {
    console.error(
      "[ServerAction:updateStudentProfile] Admin SDK init failed:",
      adminInitializationError.message,
    );
    throw new Error("Server error: Admin SDK initialization failed.");
  }
  if (!adminDb || !adminAuth) {
    console.error(
      "[ServerAction:updateStudentProfile] Admin DB or Auth not initialized.",
    );
    throw new Error("Server error: Admin services not initialized.");
  }
  ...
  try {
    ...
    console.error(
      "[ServerAction:updateStudentProfile] Invalid ID token:",
      error,
    );
    throw new Error("Authentication failed. Invalid or expired token.");
  }
  ...
  try {
    ...
    console.log(
      `[ServerAction:updateStudentProfile] Profile updated successfully for UID: ${userId}`,
    );
  } catch (error) {
    console.error(
      `[ServerAction:updateStudentProfile] Error updating profile for UID ${userId}:`,
      error,
    );
